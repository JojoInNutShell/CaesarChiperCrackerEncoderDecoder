...
def main():
    cc_text = input("""
WELCOME TO CAESAR CHIPER ENCODER DECODER BREAKER

Menu :
1)Encode Text with Caesar Chiper
2)Decode Text with Caesar Chiper
3)Break Caesar Chiper Encrypted Text

Please choose a menu by the number beside it : """)
    if cc_text == "1":
        encode_with_cc()
    elif cc_text == "2":
        decode_with_cc()
...
def encode_with_cc():
    ...
    print(f"This is the text after encoded by Caesar Chiper : {cc_text}")
    go_to_menu()
...
def decode_with_cc():
    ...
    for word in cc_text:
        if word in uppercase_alphabets:
            ...
            counter += 1
    cc_text = "".join(cc_text)
    print(f"This is the decrypted text that before encrypted by Caesar Chiper : {cc_text}")
    go_to_menu()
